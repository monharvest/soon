...
export default async function Home() {
  ...
  try {
    ...
    console.error("[v0] Error fetching posts from KV:", err)
    allPosts = []
  }
  ...
  if (!allPosts || allPosts.length === 0) {
    try {
      ...
      if (fs.existsSync(fallbackPath)) {
        ...
        console.warn("[v0] Using local posts.json fallback for Home page")
      }
    } catch (err) {
  ...
  const posts = (allPosts || []).filter((post: any) => post.published === true)
